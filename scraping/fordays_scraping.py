...
def single_product(url, brand_name = 'Fordays'):
    ...
    for i in res:
      ...
      description = [ elem.text.strip() for elem in description_res ] if len(description_res) > 0 else None

      #fabric
      fabric_res = i.find_all('span',{'itemprop':"fabric_detail"})
      fabric = [ elem.text.strip() for elem in fabric_res ] if len(fabric_res) > 0 else None
      ...
      price = [ elem.text.strip() for elem in price_list ] if len(price_list) > 0 else None
